...
검증 한계: 전체 `npm run lint`는 기존 UI 스캐폴드와 일부 기존 타입/React 규칙 경고로 통과하지 않습니다. 타입 검사, 23개 기능·기하 테스트, 프로덕션 빌드 및 로컬 HTTP 응답 확인은 통과했습니다.
...
검증: 23개 기능·기하 테스트에 한국어/영어 명령, 치아 번호·6점 매핑, 음수 GM, 출혈 없음, 추가 필드와 미입력 보존, 메시 세분화 범위 및 참고 모형 분리를 포함합니다. 실제 마이크와 WebGL 화면 검증은 사용 가능한 브라우저가 없어 수행하지 못했습니다.
