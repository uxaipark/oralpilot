  ...
  return (
    <div className={`timemachine${open ? '' : ' shut'}`}>
      <button
        ...
        onClick={onToggle}
        aria-expanded={open}
        title={open ? 'Collapse the time machine' : 'Expand the time machine'}
      >
        <span className="rail" />
        ...
        {ahead > 0 && (
          <span className="tm-behind">rewound · {ahead} ahead</span>
        )}
        <span className="tm-sub">
          drag to rewind the chart to any point — every value on screen follows
        </span>
      ...
      <div className="tm-now">
        {total === 0 ? (
          <span className="tm-empty">
            Nothing recorded yet — type a value and this fills in behind you.
          </span>
        ) : at ? (
          <>
            ...
            {ahead > 0 && (
              <span className="tm-ahead">
                {ahead} edit{ahead === 1 ? '' : 's'} ahead
              </span>
            )}
          </>
        ) : (
