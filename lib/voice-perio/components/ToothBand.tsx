...
const Ramp = ({
  ...
  id: string;
  horizontal: boolean;
  stops: ReturnType<typeof rootRamp>;
}) => (
  <linearGradient
    id={id}
    x1="0"
    ...
    <defs>
      ...
      >
        <stop
          offset="0"
          stopColor="#fff"
          stopOpacity={concave ? '.20' : '.32'}
        />
        <stop
          offset=".55"
          stopColor="#fff"
          stopOpacity={concave ? '.06' : '.09'}
        />
      ...
      >
        <stop
          offset="0"
          stopColor="#fff"
          stopOpacity={concave ? '.52' : '.82'}
        />
        <stop
          offset=".38"
          stopColor="#fff"
          stopOpacity={concave ? '.18' : '.30'}
        />
        <stop offset="1" stopColor="#fff" stopOpacity="0" />
      </radialGradient>
  ...
  for (let mm = -4; mm <= 14; mm++) {
    ...
    lines.push(
      <line
        ...
        x2={HALF_W}
        y2={y}
        stroke={strong ? 'var(--rule-5)' : 'var(--rule)'}
        strokeWidth={strong ? 0.9 : 0.6}
      />,
    );
  }
...
function PocketGraph({ band, teeth, chart, cy, dir }: LayerProps) {
  ...
  const runs = useMemo(() => {
    ...
    teeth.forEach((n, i) => {
      ...
      siteOrder(n).forEach((p, k) => {
        ...
        if (pd == null || t[band.surf].gm[p] == null) {
          if (run.length) {
            out.push(run);
            run = [];
          }
  ...
  return localize(
    <g pointerEvents="none">
      {runs.map((r, i) => {
        const gl = r
          .map((q) => `${q.x.toFixed(1)},${q.gy.toFixed(1)}`)
          .join(' ');
        const pl = r
          .map((q) => `${q.x.toFixed(1)},${q.py.toFixed(1)}`)
          .join(' ');
        const poly = `${gl} ${[...r]
          .reverse()
          .map((q) => `${q.x.toFixed(1)},${q.py.toFixed(1)}`)
          ...
          <g key={i}>
            ...
            {r.map((q, j) => (
              <g key={j}>
                ...
                {q.bop && (
                  <circle cx={q.x} cy={q.py} r={2.4} fill="var(--bop)" />
                )}
              </g>
            ))}
          </g>
...
type LayerProps = Omit<Props, 'density' | 'dispatch'> & {
  cy: number;
  dir: number;
};
...
function FurcationMarks({ band, teeth, chart, cy, dir }: LayerProps) {
  ...
  teeth.forEach((n, i) => {
    ...
    for (const p of furcationSites(n, band.surf)) {
      ...
      if (!v) continue;
      const x =
        p === 'C' ? i * COL + COL / 2 : i * COL + (ps.indexOf(p) + 0.5) * SITE;
      const color =
        v >= 3 ? 'var(--crit)' : v === 2 ? 'var(--warn)' : 'var(--ink-2)';
...
export function ToothBand({
  ...
  const cycle = (n: number) => {
    ...
    dispatch({
      type: 'setCursor',
      at: { n, surf: band.surf, p: siteOrder(n)[1] },
    });
  ...
  const dir = up ? -1 : 1;
  const signature = teeth
    .map((n) => `${chart[n].status[0]}${chart[n].crown ? 'c' : ''}`)
    .join('');
      ...
      <g filter={`url(#shd${gradKey})`} opacity={0.3} pointerEvents="none">
        {teeth.map((n, i) => {
          ...
          return (
            <g
              key={n}
              transform={`translate(${i * COL + COL / 2 + 1.5},${cy + 2.4})${up ? '' : ' scale(1,-1)'}`}
            >
              <path
                d={toothShape(n, surf).silhouette}
                fill="var(--tooth-shadow)"
              />
            </g>
          );
        })}
