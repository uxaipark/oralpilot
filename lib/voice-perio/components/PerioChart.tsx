...
function valueAt(
  ...
  if (row === 'cal') {
    ...
    const gm = t[surf].gm[p];
    return pd == null || gm == null ? null : pd + gm;
  }
...
function severity(row: RowId, v: number | null): string {
  ...
  if (row === 'furc' || row === 'mob' || row === 'gi')
    return v >= 3 ? ' v-crit' : v >= 2 ? ' v-warn' : '';
      ...
      <div
        ...
        onClick={() =>
          ...
          dispatch({
            type: 'setStatus',
            n,
            status: t.status === 'implant' ? 'present' : 'implant',
          })
        }
      >
        {t.status === 'implant' && (
      ...
      <div
        ...
        onBlur={(e) =>
          dispatch({
            type: 'setNote',
            n,
            note: e.currentTarget.textContent?.trim() ?? '',
          })
        }
      >
        {t.note}
...
function DataRow({
  ...
  cursor,
  dispatch,
}: { b: Band; row: RowId } & Pick<Props['state'], never> & {
    chart: Chart;
    cursor: Cursor;
    dispatch: (a: Action) => void;
  }) {
...
function NumbersRow({
  ...
  cursor: Cursor;
  dispatch: (a: Action) => void;
  numbering: AppState['meta']['numbering'];
  arch: 'U' | 'L';
  shown: boolean;
}) {
  const localize = useLocalize();
    ...
    list.map((n) => {
      ...
      return (
        ...
        >
          ...
          {t.status === 'implant' ? (
            <span className="tag">IMP</span>
          ) : t.crown ? (
            <span className="tag">CR</span>
          ) : null}
        </div>
      );
    });
    ...
    <>
      ...
      <div className="rl mid">
        ...
        <button
          ...
          title={
            shown
              ? `Hide the ${arch === 'U' ? 'upper' : 'lower'} teeth`
              : `Show the ${arch === 'U' ? 'upper' : 'lower'} teeth`
          }
        >
          <EyeIcon open={shown} />
          <span>tooth</span>
...
function SummaryBar({ chart }: { chart: Chart }) {
  ...
  return localize(
    <div className="summary">
      ...
      <div>
        평균 CAL <b>{m.meanCal} mm</b>
      </div>
      <div className={m.bopPct >= 30 ? 'hot' : ''}>
        BOP <b>{m.bopPct} %</b>
      </div>
      <div className={m.plqPct >= 40 ? 'hot' : ''}>
        PI <b>{m.plqPct} %</b>
      </div>
      <div>
        부위 ≥ 5 mm <b>{m.p5}</b>
...
export function PerioChart({ state, dispatch, density }: Props) {
    ...
    (row: RowId) => {
      if (entry === 'pair') {
        if (row === 'gm') dispatch({ type: 'setRow', row: 'pd' });
          ...
          dispatch({ type: 'advance', step: 1, landOn: 'gm' });
        return;
      }
      if (SWEEPING_ROWS[entry].includes(row))
        dispatch({ type: 'advance', step: 1 });
    },
    [entry, dispatch],
  );
  ...
  const onKeyDown = useCallback(
    (e: React.KeyboardEvent) => {
      ...
      const shortcut: Record<string, MarkRow> = {
        b: 'bop',
        s: 'sup',
        p: 'plq',
        c: 'clc',
      ...
      if (shortcut[low]) {
        dispatch({ type: 'toggleMark', row: shortcut[low] });
        e.preventDefault();
        return;
      }
      if (k === 'ArrowRight') {
        dispatch({ type: 'moveScreen', step: 1 });
        e.preventDefault();
        return;
      }
      if (k === 'ArrowLeft') {
      ...
      if (k === 'ArrowDown') {
        dispatch({
          type: 'moveRow',
          step: band(bandOf(cursor.n, cursor.surf)).above ? 1 : -1,
        });
        e.preventDefault();
        return;
      }
      ...
      if (k === 'Enter' || k === 'Tab') {
        dispatch({ type: 'advance', step: e.shiftKey ? -1 : 1 });
        e.preventDefault();
        return;
      }
            ...
            <Fragment key={arch}>
              ...
              {arch === 'U' && (
                <TimeMachine
                  ...
                  onToggle={() =>
                    dispatch({ type: 'togglePanel', id: 'timeMachine' })
                  }
                  dispatch={dispatch}
                />
              )}
