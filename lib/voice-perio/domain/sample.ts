...
export function sampleChart(): Chart {
  ...
  for (const n of ALL_TEETH) {
    ...
    for (const surf of ['B', 'L'] as Surface[]) {
      ...
      for (const p of siteOrder(n)) {
        ...
        o.gm[p] = rec;
        const bleeds =
          pd >= 5 ? rnd() < 0.82 : pd === 4 ? rnd() < 0.55 : rnd() < 0.18;
  ...
  hot(30, 'L', 'D', 8, 4, 2);
  chart[30].mobility = 1;
  chart[30].note = 'Endo-perio, distal';
  hot(3, 'B', 'D', 8, 3, 2);
  hot(3, 'L', 'D', 7, 3, 2);
  hot(3, 'L', 'M', 7, 2, 1);
  hot(14, 'B', 'D', 7, 2, 2);
  hot(14, 'L', 'D', 8, 3, 3);
  chart[14].mobility = 1;
  chart[14].note = 'Class III furcation, guarded';
  hot(31, 'B', 'D', 7, 2, 1);
  hot(31, 'L', 'C', 6, 2, 1);
  ...
  const imp = chart[19];
  imp.B.pd = { M: 5, C: 4, D: 5 };
  imp.B.gm = { M: 2, C: 2, D: 2 };
  imp.L.pd = { M: 5, C: 4, D: 6 };
  imp.L.gm = { M: 2, C: 2, D: 3 };
  imp.B.bop = { M: true, D: true };
  imp.L.bop = { D: true };
  imp.B.furc = {};
  imp.L.furc = {};
  imp.mobility = 0;
