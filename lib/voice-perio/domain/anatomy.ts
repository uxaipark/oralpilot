...
function lingualForm(n: number, a: Anatomy): Anatomy {
  ...
  if (a.type === 'incisor' || a.type === 'canine') {
    return {
      ...a,
      cw: a.cw * (upper ? 0.88 : 0.9),
      ch: a.ch * 0.98,
      cerv: a.cerv * 0.96,
      cingulum: upper ? 1 : 0.55,
    };
  }
  if (a.type === 'premolar') {
...
export function anatomy(n: number, surf: Surface = 'B'): Anatomy {
  ...
  if (MAXILLARY_MOLARS.includes(n) || MANDIBULAR_MOLARS.includes(n)) {
    ...
    a = {
      ...base,
      type: 'molar',
      roots: MAXILLARY_MOLARS.includes(n) ? 3 : 2,
      cw,
      ch: 25,
      rl: third ? 38 : 44,
      cerv: cw * 0.82,
    ...
    a = {
      ...base,
      type: 'premolar',
      roots: MAXILLARY_FIRST_PREMOLARS.includes(n) ? 2 : 1,
      cw: 26,
      ch: 27,
      rl: isUpper(n) ? 47 : 50,
      cerv: 26 * 0.8,
...
export function cuspCount(n: number, surf: Surface): number {
  ...
  if (MANDIBULAR_MOLARS.includes(n))
    return (n === 19 || n === 30) && surf === 'B' ? 3 : 2;
  if (PREMOLARS.includes(n))
    return surf === 'L' && (n === 20 || n === 29) ? 2 : 1;
