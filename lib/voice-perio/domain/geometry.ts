...
const scallop = (a: Anatomy) =>
  a.type === 'incisor' || a.type === 'canine' ? 2.8 : 1.8;
const occStart = (cusps: number) => (cusps === 0 ? 0.8 : 0.72);

function occlusalEdge(
  cusps: number,
  bw: number,
  h: number,
  sharp: boolean,
...
export function rootRamp(): Stop[] {
  return sampleRamp(
    (x) =>
      Math.cos(((x - 0.32) * Math.PI) / 0.56) * 0.78 +
      0.38 * bell(x, 0.97, 0.05) -
      0.26 * bell(x, 0.015, 0.04),
    0.46,
    0.26,
  );
}
