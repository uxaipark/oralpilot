...
export function useVoice(state: AppState, dispatch: (a: Action) => void) {
  ...
  const pending = useRef(
    new Map<
      number,
      { chart: AppState['chart']; cursor: string; actions: Action[] }
    >(),
  );
  ...
  const submit = useCallback(
    (raw: string, confidence: number, source: 'speech' | 'typed') => {
      ...
      for (const part of parts) {
        ...
        if (!parsed || parsed.tone === 'err') {
          ok = false;
          messages.push(parsed?.message ?? `Not understood: "${part}"`);
          break;
        }
        messages.push(parsed.message);
        queued.push(...parsed.actions);
        const next = parsed.actions.reduce((st, a) => reducer(st, a), {
          ...s,
          chart,
          cursor,
        });
      ...
      if (held && ok)
        pending.current.set(s.utteranceSeq + 1, {
          chart: s.chart,
          cursor: JSON.stringify(s.cursor),
          actions: queued,
        });
      if (!ok || held) return;
      latest.current = {
        ...queued.reduce((st, a) => reducer(st, a), s),
        utteranceSeq: s.utteranceSeq + 1,
      };
      for (const a of queued) dispatch(a);
      if (s.voice.readback && 'speechSynthesis' in window) {
        window.speechSynthesis.speak(
          new SpeechSynthesisUtterance(messages.join(', ')),
        );
      }
    },
    [dispatch],
  ...
  const confirm = useCallback(
    (id: number) => {
      ...
      if (!u) return;
      const saved = pending.current.get(id);
      if (u.outcome !== 'held') return;
      if (
        !saved ||
        saved.chart !== s.chart ||
        saved.cursor !== JSON.stringify(s.cursor)
      ) {
        dispatch({
          ...
          outcome: 'rejected',
          message:
            '차트나 선택 위치가 바뀌었습니다. 대상 부위를 다시 선택하고 입력하세요.',
        });
        pending.current.delete(id);
        return;
      }
      for (const a of saved.actions) dispatch(a);
      dispatch({ type: 'resolveUtterance', id, outcome: 'applied' });
      pending.current.delete(id);
    },
    [dispatch],
  );
  ...
  const start = useCallback(() => {
    ...
    rec.current = startRecognition(v.locale, v.processLocally, {
      ...
      onFinal: (t, c) => {
        setStatus((v) => ({ ...v, interim: '' }));
        submit(t, c, 'speech');
      },
      onError: (m) => {
        setStatus((v) => ({ ...v, error: m, listening: false }));
        rec.current = null;
      },
      onEnd: () => {
        rec.current = null;
  ...
  const dryRun = useCallback(
    (raw: string): { stages: Stage[]; message: string; tone: 'ok' | 'err' } => {
      ...
      return {
        stages,
        message: messages.join(' · ') || 'Nothing recognised in that',
        tone,
      };
    },
    [],
