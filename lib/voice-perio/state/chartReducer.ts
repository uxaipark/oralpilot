...
export const CHART_VIEWS: Array<{
  id: ChartView;
  label: string;
  hint: string;
}> = [
  { id: 'perio', label: 'Perio chart', hint: 'Six-point probing, 32 teeth' },
  {
    id: 'implant',
    label: 'Implant chart',
    hint: 'Fixtures, platforms, peri-implant probing',
  },
  {
    id: 'restorative',
    label: 'Restorative',
    hint: 'Existing and planned restorations',
  },
];
...
export const initialState = (): AppState => ({
  ...
  voice: {
    ...DEFAULT_VOICE,
    vocabulary: DEFAULT_VOICE.vocabulary.map((v) => ({ ...v })),
  },
  utterances: [],
  utteranceSeq: 0,
  log: [],
...
function seek(state: AppState, to: number): AppState {
  ...
  const cursor = edge
    ? { n: edge.n, surf: edge.surf, p: edge.p, row: edge.row }
    : state.cursor;
  return {
    ...state,
...
export type Action =
  ...
  | {
      type: 'hydrate';
      payload: Partial<AppState> & { history?: AppState['history'] };
    }
  | {
      type: 'log';
      text: string;
...
export function reducer(state: AppState, action: Action): AppState {
  ...
  switch (action.type) {
    ...
    case 'setCursor': {
      ...
      return {
        ...state,
        cursor: { ...action.at, row: rows.includes(want) ? want : 'pd' },
      };
    }

    case 'moveScreen': {
      const list = sequence(chart, 'screen').filter(
        (s) => bandOf(s.n, s.surf) === bandOf(cursor.n, cursor.surf),
      );
    ...
    case 'moveRow': {
      const rows = navigableRows(
        band(bandOf(cursor.n, cursor.surf)),
        state.optional,
      );
      ...
      return {
        ...
        cursor: {
          ...cursor,
          row: rows[(i + action.step + rows.length) % rows.length],
        },
      };
    }
    ...
    case 'advance': {
      ...
      const i = list.findIndex(
        (s) => s.n === cursor.n && s.surf === cursor.surf && s.p === cursor.p,
      );
      if (i < 0) return state;
      const next =
        list[(((i + action.step) % list.length) + list.length) % list.length];
      const b = band(bandOf(next.n, next.surf));
      const rows = navigableRows(b, state.optional);
      const want = action.landOn ?? cursor.row;
      return {
        ...state,
        cursor: { ...next, row: rows.includes(want) ? want : 'pd' },
      };
    }
    ...
    case 'setValue': {
      if (action.row === 'mob') {
        ...
        return commit(
          state,
          patchTooth(chart, cursor.n, (t) => ({
            ...t,
            mobility: action.value,
          })),
          cursor.n,
      ...
      if (action.row === 'cal') {
        ...
        return commit(
          state,
          writeSite(chart, cursor.n, cursor.surf, 'gm', cursor.p, gm),
          cursor.n,
      ...
      return commit(
        state,
        writeSite(chart, cursor.n, cursor.surf, action.row, p, action.value),
        cursor.n,
    ...
    case 'clearValue': {
      ...
      if (cursor.row === 'mob')
        return commit(
          state,
          patchTooth(chart, cursor.n, (t) => ({ ...t, mobility: null })),
          cursor.n,
          label,
        );
      if (cursor.row === 'cal')
        return commit(
          state,
          writeSite(chart, cursor.n, cursor.surf, 'gm', cursor.p, null),
          cursor.n,
          'CAL cleared',
        );
      if (
        ...
        return commit(
          state,
          toggleMark(chart, cursor.n, cursor.surf, cursor.row, cursor.p, false),
          cursor.n,
          label,
        );
    ...
    case 'toggleMark': {
      ...
      return commit(
        ...
        next,
        cursor.n,
        `${ROWS[action.row].label} ${on ? 'on' : 'off'}`,
      );
    }
    ...
    case 'applyTriplet': {
      ...
      siteOrder(cursor.n).forEach((p, i) => {
        ...
        if (v != null)
          next = writeSite(next, cursor.n, cursor.surf, action.row, p, v);
      });
      return commit(
        state,
        next,
        cursor.n,
        `${ROWS[action.row].label} ${action.values.join(' ')}`,
      );
    }
    ...
    case 'setStatus':
      return commit(
        state,
        patchTooth(chart, action.n, (t) => ({ ...t, status: action.status })),
        action.n,
    ...
    case 'toggleCrown':
      return commit(
        state,
        patchTooth(chart, action.n, (t) => ({ ...t, crown: !t.crown })),
        action.n,
    ...
    case 'cycleTooth': {
      ...
      const name =
        to.status === 'present' ? (to.crown ? 'crown' : 'present') : to.status!;
      return commit(
        state,
        patchTooth(chart, action.n, (tt) => ({ ...tt, ...to })),
        action.n,
        `Cycled to ${name}`,
      );
    }
    ...
    case 'setNote':
      ...
      return commit(
        state,
        patchTooth(chart, action.n, (t) => ({ ...t, note: action.note })),
        action.n,
    ...
    case 'setRecClass':
      return commit(
        state,
        patchTooth(chart, action.n, (t) => ({
          ...t,
          recClass: action.recClass,
        })),
        action.n,
        action.recClass
          ? `Recession ${action.recClass}`
          : 'Recession class cleared',
      );
    ...
    case 'toggleOptional': {
      const optional = {
        ...state.optional,
        [action.row]: !state.optional[action.row],
      };
      const rows = navigableRows(band(bandOf(cursor.n, cursor.surf)), optional);
      return {
        ...state,
        optional,
        cursor: rows.includes(cursor.row) ? cursor : { ...cursor, row: 'pd' },
      };
    }
    ...
    case 'setRow': {
      ...
      return rows.includes(action.row)
        ? { ...state, cursor: { ...cursor, row: action.row } }
        : state;
    }
    ...
    case 'setTeethShown': {
      if (action.arch) {
        ...
        return {
          ...state,
          teethShown: { ...state.teethShown, [action.arch]: shown },
        };
      }
    ...
    case 'togglePanel':
      return {
        ...
        panels: {
          ...state.panels,
          [action.id]: !(state.panels[action.id] ?? true),
        },
      };
      ...
      return {
        ...
        voice: {
          ...
          vocabulary: [
            ...
            { ...action.rule, id: `v${Date.now().toString(36)}` },
          ],
        },
      };
    ...
    case 'removeVocab':
      return {
        ...
        voice: {
          ...state.voice,
          vocabulary: state.voice.vocabulary.filter((v) => v.id !== action.id),
        },
      };
    ...
    case 'pushUtterance':
      return {
        ...
        utterances: [
          { ...action.utterance, id: state.utteranceSeq + 1 },
          ...state.utterances,
        ].slice(0, 200),
      };
        ...
        utterances: state.utterances.map((u) =>
          u.id === action.id
            ? {
                ...
                message: action.message ?? u.message,
              }
            : u,
        ),
      };
    ...
    case 'resetToSample': {
      ...
      return {
        ...
        log: [
          {
            text: 'Everything reset — sample exam and default settings reloaded',
            tone: 'ok' as const,
