...
Direction markers (L/R/S/I/A/P), clinical acronyms, measurement units and case IDs are invariant notation. `translate="no"` additionally protects the direction widget, original case names and user-authored text. Compact chart rows use standard abbreviations (PD, GM, CAL, BOP, MOB, FUR, IMP) with full translated names in accessible labels/tooltips. The September 2026 audit reviewed the full catalog and corrected literal translation collisions (개, 회, 동요, 상악동, tooth classes, recession classification and imaging terminology), removed code/CSS literals from the catalog, and verified all six workflows in English and Japanese for Korean text left behind.
